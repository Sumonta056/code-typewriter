...
async function onLoadFromUrl(input: string) {
  ...
  if (!parsed) {
    urlError.value = true
    setTimeout(() => { urlError.value = false }, 1500)
    return
  }
...
@media (min-width: 1200px) {
  ...
  .pp-sidebar :deep(.logo-terminal) { display: none; }
  .pp-sidebar :deep(.logo-text) { font-size: 0.95rem; }
  .pp-sidebar :deep(.logo-version) { align-self: flex-start; }
